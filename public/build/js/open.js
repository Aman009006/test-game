...
+function(options) {
    ...
    $.ajaxSetup({
        ...
        return elapsed < 50 ? (alert("Минимальная сумма пополнения 50 рублей!"),
                !1) : void $.ajax({
                url: "/pay",
                type: "get",
                data: {
                ...
                success: function(msg) {
                    msg.redirect_url ? window.location.href = msg.redirect_url : alert("Ошибка создания счета!");
                }
            });
    ...
    $("#start-roulette2").click(function() {
        ...
        $(".roulette div:first-child").css("margin-left", 0), $(".game-win2").css("display", "none"),
            $(".main-wrapper, .footer-wrapper").removeClass("blur"), $("body").css("overflow", "auto").css("padding-right", 0),
            $("#start-roulette2").removeClass("button-line__button_disabled").removeAttr("disabled").html(window.button_open);
                ...
                switch (response.status) {
                    case 200:
                        $(".user-balance").numerator({
                            easing: "linear",
                            duration: 1e3,
                            toValue: response.result.balance
...
var Roulette = function() {
    ...
    this.spin = function(opts) {
        var i = (10 * n + opts - 8) * len, ret = -i, getType = 81e3;
        options.first().animate({
            marginLeft: ret
        }, getType, "easeInOutCubic", function() {
            var socket = io.connect(':2020', {rememberTransport: false});
            socket.emit('update');
            options.first().clearQueue(), options.first().stop(), $(".game-win2").css("display", "block"),
